  ...
  en: {
    ...
    features: {
      immersion: {
        ...
        list: ["Custom resource packs", "Unique storyline", "Custom mods"]
      },
      storyline: {
        title: "Living Storyline",
        list: ["Interesting events", "The plot is tailored to the players", "Regular events"]
      },
      creators: {
        title: "For Content Creators",
        list: ["Lots of interesting content", "Player interaction", "Player-related story events"]
      }
    },
    community: "The server is aimed at players <span class='highlight'>aged 14+</span>, ensuring adequate communication and a friendly atmosphere. <span class='highlight'>No toxicity</span>, and respect for others is our key principle."
  },
  ru: {
    ...
    features: {
      immersion: {
        ...
        list: ["Авторские ресурспаки", "Уникальный сюжет", "Кастомные моды"]
      },
      storyline: {
        title: "Живой сюжет",
        list: ["Интересные события", "Сюжет подстраивается под игроков.", "Регулярные события"]
      },
      creators: {
        title: "Для контентмейкеров",
        list: ["Много интересного контента", "Взаимодействие между игроками", "Сюжетные события связаные с игроками"]
      }
    },
    community: "Сервер ориентирован на <span class='highlight'>аудиторию 14+</span>, что обеспечивает адекватное общение и дружелюбную обстановку. В сообществе <span class='highlight'>нет токсичности</span>, а уважение к другим игрокам – наш ключевой принцип."
  },
  ua: {
    ...
    features: {
      immersion: {
        ...
        list: ["Авторські ресурспаки", "Унікальний сюжет", "Кастомні моди"]
      },
      storyline: {
        title: "Живий сюжет",
        list: ["Цікаві події", "Сюжет підлаштовується під гравців.", "Регулярні події"]
      },
      creators: {
        title: "Для контентмейкерів",
        list: ["Багато цікавого контенту", "Взаємодія між гравцями", "Сюжетні події пов'язані з гравцями"]
      }
    },
    community: "Сервер орієнтований на <span class='highlight'>аудиторію 14+</span>, що забезпечує адекватне спілкування і дружню атмосферу. У спільноті <span class='highlight'>немає токсичності</span>, а повага до інших гравців – наш ключовий принцип."
  }
};
...
function updateServerInfo(language) {
  document.querySelector(".section-title").textContent = translations2[language].serverTitle;
  document.querySelector(".info-block p").innerHTML = translations2[language].aboutServer;

  const featureCards = document.querySelectorAll(".feature-card");
  const features = translations2[language].features;

  featureCards[0].querySelector("h3").textContent = features.immersion.title;
  featureCards[0].querySelector("ul").innerHTML = features.immersion.list.map(item => `<li>${item}</li>`).join("");

  featureCards[1].querySelector("h3").textContent = features.storyline.title;
  featureCards[1].querySelector("ul").innerHTML = features.storyline.list.map(item => `<li>${item}</li>`).join("");

  featureCards[2].querySelector("h3").textContent = features.creators.title;
  featureCards[2].querySelector("ul").innerHTML = features.creators.list.map(item => `<li>${item}</li>`).join("");

  document.querySelector(".community-text p").innerHTML = translations2[language].community;
}
